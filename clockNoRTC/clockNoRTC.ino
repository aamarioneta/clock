...
<body>
    <input onchange='update(this.jscolor)' value="ffcc00" class="jscolor {width:800, height:600}">
    <div class="grid-container">
        <div class="grid-item">red:</div>
        <div class="grid-item"><input id='r'></div>
        <div class="grid-item">green:</div>
        <div class="grid-item"><input id='g'></div>
        <div class="grid-item">blue:</div>
        <div class="grid-item"><input id='b'></div>
        <div class="grid-item">brightness divide by:</div>
        <div class="grid-item"><input id='l' value='4'></div>
        <div class="grid-item" id='rect'>
            <button onclick='setColor()'>Apply</button>
            <a href='/t'><button>Update Time</button></a>
        </div>
    </div>
    </div>
    <script>function setColor() { r = document.getElementById('r').value; g = document.getElementById('g').value; b = document.getElementById('b').value; l = document.getElementById('l').value; window.location.href = "/?r=" + Math.round(r) + "&g=" + Math.round(g) + "&b=" + Math.round(b) + "&l=" + Math.round(l) } function update(jscolor) { console.log(jscolor.rgb[0]); console.log(jscolor.rgb[1]); console.log(jscolor.rgb[2]); document.getElementById('r').value = jscolor.rgb[0]; document.getElementById('g').value = jscolor.rgb[1]; document.getElementById('b').value = jscolor.rgb[2]; document.getElementById('rect').style.backgroundColor = '#' + jscolor }</script>
</body>
</html>
)=====";

WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP, "openwrt.lan");
